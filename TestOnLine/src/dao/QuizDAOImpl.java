...
public class QuizDAOImpl implements QuizDAO {
	...
	public Quiz select(Connection conn, int id) throws Exception {
		String sql = "select quiz.*,argomento.descrizione from quiz join argomento on quiz.id_argomento=argomento.id where quiz.id = ?";
		PreparedStatement ps = conn.prepareStatement(sql);
		ps.setInt(1, id);
		...
		if (rs.next()) {
			...
			Argomento arg = new Argomento();
			arg.setId(rs.getInt("id_argomento"));
			arg.setDescrizione(rs.getString("descrizione"));
			String difficolta = rs.getString("difficolta");
